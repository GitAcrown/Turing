...
ACTIONS_REPR = {
    ...
    "CMUTE"   : ("Mute du channel", "\N{SPEAKER WITH CANCELLATION STROKE}"),
    "SMUTE"   : ("Mute du serveur", "\N{SPEAKER WITH CANCELLATION STROKE}"),
    "SOFTBAN" : ("Softban", "\N{DASH SYMBOL} \N{HAMMER}"),
    "HACKBAN" : ("Ban préventif", "\N{BUST IN SILHOUETTE} \N{HAMMER}"),
    "UNBAN"   : ("Déban", "\N{DOVE OF PEACE}")
}
...
class Mod:
    ...
    async def modset(self, ctx):
        """Gestionnaire des paramètres serveurs de modération."""
        if ctx.invoked_subcommand is None:
            ...
            msg = ("Admin role: {ADMIN_ROLE}\n"
                   "Mod role: {MOD_ROLE}\n"
                   "Mod-log: {mod-log}\n"
                   "Supprimer les répétitions: {delete_repeats}\n"
                   "Bannir les spams de mention: {ban_mention_spam}\n"
                   "Délai de suppression: {delete_delay}\n"
                   "Respect de la hiérarchie: {respect_hierarchy}"
                   "".format(**_settings))
    ...
    async def modlog(self, ctx, channel : discord.Channel=None):
        """Attribue un channel aux logs de modération

        Ne pas donner de serveur reset cette valeur"""
        server = ctx.message.server
        if channel:
            self.settings[server.id]["mod-log"] = channel.id
            await self.bot.say("Les évenements de modérations seront envoyés sur {}"
    ...
    async def banmentionspam(self, ctx, max_mentions : int=False):
        ...
        if max_mentions:
            ...
            await self.bot.say("Autoban pour le spam de mention activé. "
                               "Ceux qui mentionneront {} ou plus de personnes "
                               "dans un seul message seront bannis automatiquement."
                               "".format(max_mentions))
        else:
    ...
    async def deleterepeats(self, ctx):
        ...
        if not self.settings[server.id]["delete_repeats"]:
            ...
            await self.bot.say("Les messages répétés plus de 3 fois seront supprimés.")
        else:
            self.settings[server.id]["delete_repeats"] = False
            await self.bot.say("Les messages répétés seront ignorés.")
        dataIO.save_json("data/mod/settings.json", self.settings)
    ...
    async def resetcases(self, ctx):
        ...
        await self.bot.say("Les casiers ont été reset.")
    ...
    async def deletedelay(self, ctx, time: int=None):
        ...
        if time is not None:
            ...
            if time == -1:
                await self.bot.say("Le délai à été retiré.")
            else:
                await self.bot.say("Un délai de {}"
                                   " secondes se déroulera avant la suppression de la commande.".format(time))
            dataIO.save_json("data/mod/settings.json", self.settings)
        else:
            try:
                delay = self.settings[server.id]["delete_delay"]
            except KeyError:
                await self.bot.say("Aucun délai n'a été mis sur ce serveur")
            else:
                if delay != -1:
                    await self.bot.say("Le bot supprimera les commandes après"
                                       " {} secondes. Mettre cette valeur sur -1 désactive cette fonctionnalité".format(delay))
                else:
                    await self.bot.say("Je ne supprimerai plus les commandes.")
    ...
    async def set_cases(self, ctx, action: str = None, enabled: bool = None):
        ...
        if action == enabled:  # No args given
            ...
            msg = "Ce n'est pas une action valide. Les actions valides sont: \n"
            ...
            value = self.settings[server.id].get(action,
                                                 default_settings[action])
            await self.bot.say('La création de casiers pour %s est actuellement %s' %
                               (action, 'activé' if value else 'désactivé'))
        else:
    ...
    async def hierarchy(self, ctx):
        ...
        if not toggled:
            ...
            await self.bot.say("La hiérarchie des rôles sera pris en compte dans l'exécution de commandes.")
        else:
            self.settings[server.id]["respect_hierarchy"] = False
            await self.bot.say("La hiérarchie des rôles sera ignoré dans l'exécution de commandes.")
        dataIO.save_json("data/mod/settings.json", self.settings)
    ...
    async def kick(self, ctx, user: discord.Member, *, reason: str = None):
        ...
        if author == user:
            await self.bot.say("Impossible de vous laisser faire ça \N{PENSIVE FACE}")
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("Impossible, vous êtes moins haut dans la hiérarchie que la personne que vous tentez de kick...")
            return
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("Kick réalisé.")
    ...
    async def ban(self, ctx, user: discord.Member, days: str = None, *, reason: str = None):
        ...
        if author == user:
            ...
            await self.bot.say("Impossible, vous êtes moins haut dans la hiérarchie que la personne que vous tentez de ban...")
            return
        ...
        try:
            ...
            logger.info("{}({}) a banni {}({}), supprimant {} jours de messages".format(
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("Ban réalisé.")
        except discord.errors.Forbidden:
            await self.bot.say("Je ne suis pas autorisé à faire cela.")
        except Exception as e:
            print(e)
    ...
    async def hackban(self, ctx, user_id: int, *, reason: str = None):
        ...
        if is_banned:
            await self.bot.say("Ce membre est déjà banni.")
            return
        ...
        try:
            ...
            await self.bot.say("Membre introuvable. Vérifiez l'ID.")
        except discord.Forbidden:
            await self.bot.say("Je n'ai pas les permissions pour réaliser cette action.")
        else:
            logger.info("{}({}) a banni préventivement {}"
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("Hackban réalisé. Ce membre ne pourra **jamais** rentrer sur ce serveur.")
    ...
    async def softban(self, ctx, user: discord.Member, *, reason: str = None):
        ...
        if author == user:
            ...
            await self.bot.say("Impossible, vous êtes moins haut dans la hiérarchie que la personne que vous tentez de softban..")
            return
        ...
        if can_ban:
            try:
                try:  # We don't want blocked DMs preventing us from banning
                    msg = await self.bot.send_message(user, "Tu as été banni·e puis débanni·e furtivement dans le but "
                                                            "d'effacer rapidement toute trace de ton passage sur le "
                                                            "serveur. Tu peux désormais revenir.{}".format(invite))
                except:
                    pass
                self.temp_cache.add(user, server, "BAN")
                await self.bot.ban(user, 1)
                logger.info("{}({}) a softban {}({}), supprimant 1 jour "
                    "de messages".format(author.name, author.id, user.name,
                     user.id))
                await self.new_case(server,
                                    action="SOFTBAN",
                                    mod=author,
                ...
                await self.bot.say("Softban réalisé.")
            except discord.errors.Forbidden:
                await self.bot.say("Mon rôle ne me permet pas cette action")
                await self.bot.delete_message(msg)
            except Exception as e:
                print(e)
        else:
            await self.bot.say("Impossible de faire ça.")
    ...
    async def rename(self, ctx, user : discord.Member, *, nickname=""):
        ...
        try:
            ...
            await self.bot.say("Je n'ai pas les permissions nécessaire à ce type d'action")
    ...
    async def channel_mute(self, ctx, user : discord.Member, *, reason: str = None):
        ...
        if overwrites.send_messages is False:
            await self.bot.say("Ce membre ne peut pas envoyer de message sur ce salon.")
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("Vous êtes inférieur hiérarchiquement à cette personne, c'est donc impossible.")
            return
        ...
        try:
            ...
            await self.new_case(server,
                                ...
                                user=user,
                                reason=reason)
            await self.bot.say("Ce membre est désormais mute sur ce salon.")
    ...
    async def server_mute(self, ctx, user : discord.Member, *, reason: str = None):
        ...
        if not register:
            await self.bot.say("Ce membre est déjà mute sur tous les salons.")
            return
        ...
        await self.new_case(server,
                            ...
                            user=user,
                            reason=reason)
        await self.bot.say("Ce membre est désormais mute sur l'ensemble du serveur.")
    ...
    async def channel_unmute(self, ctx, user : discord.Member):
        ...
        if overwrites.send_messages:
            await self.bot.say("Ce membre ne semble pas muté sur ce salon.")
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("Vous êtes inférieur hiérarchiquement à cette personne, c'est donc impossible.")
            return
        ...
        try:
            ...
            await self.bot.say("Je n'ai pas les permissions nécessaires à cette action.")
        else:
            try:
            ...
            dataIO.save_json("data/mod/perms_cache.json", self._perms_cache)
            await self.bot.say("Le membre a été démuté sur ce salon.")
    ...
    async def server_unmute(self, ctx, user : discord.Member):
        ...
        if user.id not in self._perms_cache:
            await self.bot.say("Ce membre n'a pas été muté avec la commande {0}mute. "
                               "Démutez-le si vous le souhaitez avec `{0}unmute <membre>`"
                               "".format(ctx.prefix))
            return
        elif not self.is_allowed_by_hierarchy(server, author, user):
            await self.bot.say("Vous êtes inférieur hiérarchiquement à cette personne, c'est donc impossible.")
        ...
        if user.id in self._perms_cache and not self._perms_cache[user.id]:
            del self._perms_cache[user.id]  # cleanup
        dataIO.save_json("data/mod/perms_cache.json", self._perms_cache)
        await self.bot.say("Ce membre est désormais démute sur ce serveur.")
    ...
    async def text(self, ctx, text: str, number: int):
        ...
        logger.info("{}({}) a supprimé {} messages "
                    " contenant '{}' sur le salon {}".format(author.name,
    ...
    async def after(self, ctx, message_id : int):
        ...
        if not is_bot:
            await self.bot.say("Cette commande n'est disponible qu'avec des comptes BOT.")
            return
        ...
        logger.info("{}({}) a supprimé {} messages dans le salon {}"
    ...
    async def cleanup_bot(self, ctx, number: int):
        ...
        if isinstance(prefixes, str):
            ...
            if asyncio.iscoroutine(prefixes):
                await self.bot.say('Les préfixes de coroutine ne sont pas encore implémentés')
                return
        ...
        logger.info("{}({}) a supprimé {} "
                    " messages de commandes sur le salon {}"
                    "".format(author.name, author.id, len(to_delete),
                              channel.name))
    ...
    async def cleanup_self(self, ctx, number: int, match_pattern: str = None):
        ...
        logger.info("{}({}) a supprimé {} messages "
                    "envoyés par le bot sur le salon {}"
                    "".format(author.name, author.id, len(to_delete),
                              channel_name))
    ...
    @commands.command(aliases=["raison"], pass_context=True)
    @checks.mod_or_permissions(manage_messages=True)
    async def reason(self, ctx, case, *, reason : str=""):
        ...
        try:
            ...
            await self.bot.say("Ce cas n'est pas le votre.")
        except KeyError:
            await self.bot.say("Ce cas n'existe pas.")
        except NoModLogChannel:
            await self.bot.say("Aucun salon de log n'est réglé.")
        except CaseMessageNotFound:
            await self.bot.say("Je ne trouve pas ce message de log.")
        except NoModLogAccess:
            await self.bot.say("Je ne suis pas autorisé à voir le message de log auquel vous faîtes référence.")
        else:
    ...
    async def ignore_channel(self, ctx, channel: discord.Channel=None):
        ...
        if not channel:
            if current_ch.id not in self.ignore_list["CHANNELS"]:
                ...
                await self.bot.say("Salon ajouté à la liste des salons ignorés.")
            else:
                await self.bot.say("Ce salon est déjà ignoré.")
        else:
            if channel.id not in self.ignore_list["CHANNELS"]:
                self.ignore_list["CHANNELS"].append(channel.id)
                dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
                await self.bot.say("Salon ajouté à la liste.")
            else:
                await self.bot.say("Salon déjà présent dans la liste.")
    ...
    async def ignore_server(self, ctx):
        ...
        if server.id not in self.ignore_list["SERVERS"]:
            ...
            dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
            await self.bot.say("Ce serveur est désormais ignoré.")
        else:
            await self.bot.say("Ce serveur est déjà ignoré.")
    ...
    async def unignore_channel(self, ctx, channel: discord.Channel=None):
        ...
        if not channel:
            if current_ch.id in self.ignore_list["CHANNELS"]:
                ...
                dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
                await self.bot.say("Ce salon n'est désormais plus ignoré.")
            else:
                await self.bot.say("Ce salon n'était pas ignoré.")
        else:
            if channel.id in self.ignore_list["CHANNELS"]:
                self.ignore_list["CHANNELS"].remove(channel.id)
                dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
                await self.bot.say("Salon désormais ignoré.")
            else:
                await self.bot.say("Ce salon n'était pas ignoré.")
    ...
    async def unignore_server(self, ctx):
        ...
        if server.id in self.ignore_list["SERVERS"]:
            ...
            dataIO.save_json("data/mod/ignorelist.json", self.ignore_list)
            await self.bot.say("Ce serveur n'est désormais plus ignoré.")
        else:
            await self.bot.say("Ce serveur n'était pas ignoré.")
        ...
        if ctx.invoked_subcommand is None:
            ...
            if server.id in self.filter:
                if self.filter[server.id]:
                    ...
                    words = "Filtrés sur ce serveur :\n\n" + words
                    try:
                        for page in pagify(words, delims=[" ", "\n"], shorten_by=8):
                            await self.bot.send_message(author, page)
                    except discord.Forbidden:
                        await self.bot.say("Je ne peux pas vous MP.")
    ...
    async def filter_remove(self, ctx, *words: str):
        ...
        if removed:
            ...
            await self.bot.say("Ces mots ne sont pas filtrés.")
    ...
    async def colour(self, ctx, role: discord.Role, value: discord.Colour):
        ...
        try:
            ...
            logger.info("{}({}) a changé la couleur du rôle '{}'".format(
                author.name, author.id, role.name))
            await self.bot.say("Changement effectué.")
        except discord.Forbidden:
            await self.bot.say("Je n'ai pas le droit de réaliser cette action.")
        except Exception as e:
            print(e)
            await self.bot.say("Quelque chose cloche, je n'ai pas réussi à faire cette action.")
    ...
    async def names(self, user : discord.Member):
        ...
        if nicks:
            if msg:
                msg += "\n\n"
            msg += "**20 dernirs surnoms**:\n"
            msg += ", ".join(nicks)
        if msg:
            await self.bot.say(msg)
        else:
            await self.bot.say("Je n'ai enregistré aucun changement pour ce membre.")
    ...
    def format_case_msg(self, case):
        ...
        if case["reason"] is None:
            tmp["reason"] = "Tapez [p]raison %i <reason> pour l'ajouter" % tmp["case"]
        if case["moderator"] is None:
            tmp["moderator"] = "Inconnu"
            tmp["moderator_id"] = "Personne n'en a réclamé la responsabilité"
        ...
        case_msg = (
            "**Cas #{case}** | {action}\n"
            "**Membre :** {user} ({user_id})\n"
            "**Modérateur :** {moderator} ({moderator_id})\n"
        ).format(**tmp)
        ...
        if server.id in self.filter.keys():
            for w in self.filter[server.id]:
                if w in message.content.lower():
                    try:
                        await self.bot.delete_message(message)
                        logger.info("Message supprimé sur {}."
                                    "Filtré : {}"
                                    "".format(server.id, w))
                        return True
                    except:
        ...
        if self.settings[server.id]["ban_mention_spam"]:
            ...
            if len(mentions) >= max_mentions:
                try:
                    ...
                    logger.info("Impossible de bannir un membre pour spam {}".format(server.id))
                else:
                    await self.new_case(server,
                                        action="BAN",
                                        mod=server.me,
    ...
    async def on_command(self, command, ctx):
        ...
        async def _delete_helper(bot, message):
            try:
                ...
                logger.debug("Suppression de message de commande sur {}".format(message.id))
            except:
